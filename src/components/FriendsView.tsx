...
import { collection, query, where, getDocs, doc, setDoc, deleteDoc, updateDoc, arrayUnion, arrayRemove } from 'firebase/firestore';
            ...
            {searchResults.map(result => (
              <div key={result.uid} className="flex items-center justify-between p-3 rounded-lg bg-zinc-500/5">
                <div className="flex items-center gap-3">
                  <img src={result.avatarUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${result.uid}`} alt="Avatar" className="w-10 h-10 rounded-full" referrerPolicy="no-referrer" />
                  <div>
          ...
          <div className="space-y-2">
            {requests.map(req => (
              <div key={req.id} className="flex items-center justify-between p-3 rounded-lg bg-zinc-500/5">
                <div className="flex items-center gap-3">
                  <img src={req.senderAvatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${req.senderId}`} alt="Avatar" className="w-10 h-10 rounded-full" referrerPolicy="no-referrer" />
      ...
      <div className={clsx(
        ...
        {friends.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {friends.map(friend => (
              <div key={friend.uid} className="flex items-center justify-between p-4 rounded-lg bg-zinc-500/5">
                <div className="flex items-center gap-3">
                  <img src={friend.avatarUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${friend.uid}`} alt="Avatar" className="w-12 h-12 rounded-full" referrerPolicy="no-referrer" />
                  <div>
                    <p className="font-medium">{friend.name}</p>
                    <p className="text-xs opacity-70">{friend.region}</p>
                  </div>
                </div>
                <button onClick={() => removeFriend(friend.uid)} className="p-2 text-red-500 hover:bg-red-500/10 rounded-lg" title="Remove Friend">
